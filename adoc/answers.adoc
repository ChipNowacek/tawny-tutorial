...
Clojure), or to our own private repo. This also separates the ontology
...
versions, two branches whatever. The only exception to this are the EDN files
...
* For raw, unpatternised ontology tawny takes about 2x as reading OWL/XML
...
suspect it is that Protege is not entirely happy with having its data
...
can reuse tooling. We can use leiningen to show us a dependency graph, we can
look for version conflicts, we can exclude duplicates from the transitive
closure.
...
all. But it fulfils the SLOD (significant load of dependencies) principle which
